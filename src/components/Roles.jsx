...
// কার্ডের রঙ (হোভার ব্যাকগ্রাউন্ড + নিচের এক্সেন্ট লাইন) এখন অটোমেটিক
// পালা করে বসে — এডমিন থেকে এটা আলাদা করে সেট করার দরকার নেই
const ROLE_THEMES = [
  { hoverBg: 'hover:bg-yellow-500/6', accentFrom: 'from-yellow-400', accentTo: 'to-yellow-600' },
  { hoverBg: 'hover:bg-green-500/6',  accentFrom: 'from-green-400',  accentTo: 'to-green-600'  },
]
...
export default function Roles() {
  ...
  const ref = useReveal()
  return (
    <section id="dayitto" ref={ref} className="opacity-0 translate-y-7 transition-[opacity,transform] duration-[650ms] ease-[cubic-bezier(.22,1,.36,1)] py-14
      ...
      <div className="flex flex-wrap justify-center gap-5">
        {roles.map((r, i) => {
          ...
          return (
            <div key={i} style={{ animationDelay: `${i * 90}ms` }} className={`stagger-item w-full sm:w-[calc(50%-0.625rem)] md:w-[calc(33.333%-0.834rem)] relative overflow-hidden bg-white dark:bg-slate-900 border border-slate-900/7 dark:border-white/8 rounded-2xl
              ...
              <div className="font-['Tiro_Bangla'] text-[1.2rem] text-slate-900 dark:text-slate-100 mb-2.5">{r.title}</div>
              <p className="text-[.875rem] leading-7 text-slate-500 dark:text-slate-400">{r.desc}</p>
              <div className={`absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r ${theme.accentFrom} ${theme.accentTo} via-transparent
