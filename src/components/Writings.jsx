      ...
      <div className="flex flex-col gap-3">
        {writings.map((w, i) => (
          <a key={i} href={w.href || '#'} style={{ animationDelay: `${i * 90}ms` }}
            className="stagger-item flex items-center gap-3.5 px-4 py-4 border border-slate-900/7 dark:border-white/8 rounded-[13px]
              bg-white dark:bg-slate-900 no-underline text-inherit transition-all duration-300
            ...
            <span className={`text-[.7rem] font-bold tracking-[.12em] uppercase flex-shrink-0 w-[70px] ${w.gold ? 'text-gold' : 'text-green-500'}`}>
              {w.type}
            </span>
            <div className="flex-1 min-w-0">
              <div className="font-['Tiro_Bangla'] text-[1.05rem] text-slate-900 dark:text-slate-100 mb-0.5">{w.title}</div>
              <div className="text-[.8rem] text-slate-400 dark:text-slate-500">{w.sub}</div>
            </div>
